...
OPENAI_BACKOFF_SEC = int(os.getenv("OPENAI_BACKOFF_SEC", "1800"))
OPENAI_RETRY_AFTER_TS = 0.0
...
def is_openai_quota_error(exc: Exception) -> bool:
    msg = (repr(exc) or "").lower()
    return "insufficient_quota" in msg or "exceeded your current quota" in msg


def heuristic_ai_payload(scenario_hint: str, message_text: str, reason: str = "rule_based_fallback") -> Dict[str, Any]:
    language = detect_language(message_text)

    mapping = {
        "lead_search": ("lead_search_reply", 0.98),
        "lead_question": ("lead_question_reply", 0.95),
        "partner_services": ("partner_pitch", 0.95),
    }

    action, confidence = mapping.get((scenario_hint or "").strip(), ("skip", 0.0))
    reply_text = fallback_reply(scenario_hint, language) if action != "skip" else ""
...
async def ai_generate_reply(
    ...
    global OPENAI_RETRY_AFTER_TS

    if not openai_client:
        return heuristic_ai_payload(scenario_hint, message_text, reason="no_openai_client")

    if OPENAI_RETRY_AFTER_TS and time.time() < OPENAI_RETRY_AFTER_TS:
        return heuristic_ai_payload(scenario_hint, message_text, reason="openai_backoff_active")
    ...
    try:
        ...
        if is_openai_quota_error(e):
            if time.time() >= OPENAI_RETRY_AFTER_TS:
                OPENAI_RETRY_AFTER_TS = time.time() + OPENAI_BACKOFF_SEC
                logging.warning(
                    "OpenAI disabled for %ss due to insufficient_quota; using local fallback",
                    OPENAI_BACKOFF_SEC,
                )
            return heuristic_ai_payload(scenario_hint, message_text, reason="openai_insufficient_quota")
    ...
    try:
        ...
        return heuristic_ai_payload(scenario_hint, message_text, reason="openai_request_failed")
