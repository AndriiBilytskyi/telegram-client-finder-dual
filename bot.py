...
AUTO_INVITE_HIGH_CONFIDENCE = os.getenv("AUTO_INVITE_HIGH_CONFIDENCE", "0").strip() == "1"
...
INVITED_FILE = os.path.join(DATA_DIR, "invited_users.json")
...
INVITED_USERS = load_json(INVITED_FILE, {})
...
def invited_user_keys(lead: Dict[str, Any]) -> List[str]:
    keys: List[str] = []

    sender_id = lead.get("sender_id")
    sender_username = (lead.get("sender_username") or "").strip().lstrip("@").lower()

    if sender_id is not None:
        keys.append(f"id:{sender_id}")

    if sender_username:
        keys.append(f"u:{sender_username}")

    return keys


def is_user_already_invited(lead: Dict[str, Any]) -> bool:
    return any(key in INVITED_USERS for key in invited_user_keys(lead))


async def remember_invited_user(lead: Dict[str, Any], source: str = "invite"):
    payload = {
        "saved_at": now_iso(),
        "source": source,
        "lead_id": lead.get("id"),
        ...
        "sender_username": lead.get("sender_username"),
        "sender_name": lead.get("sender_name"),
        "chat_title": lead.get("chat_title"),
        "message_link": lead.get("message_link"),
    }

    async with PERSIST_LOCK:
        for key in invited_user_keys(lead):
            INVITED_USERS[key] = payload
        save_json(INVITED_FILE, INVITED_USERS)
...
async def invite_lead_to_group(client: TelegramClient, lead_id: str) -> str:
    ...
    if is_user_already_invited(lead):
        lead["status"] = "already_invited"
        await remember_lead(lead)
        return f"ℹ️ User already stored as invited: {lead_id}"
    ...
    try:
        ...
        lead["status"] = "invited"
        await remember_lead(lead)
        await remember_invited_user(lead, source="invite_success")

        return f"✅ Invited {lead_id} to {TARGET_INVITE_GROUP}"
    except UserAlreadyParticipantError:
        lead["last_invite_at"] = now_iso()
        lead["status"] = "already_in_group"
        await remember_lead(lead)
        await remember_invited_user(lead, source="already_participant")
        return "ℹ️ User is already a participant"
    except UserNotMutualContactError:
...
async def handle_candidate_message(client: TelegramClient, config: Dict[str, Any], event):
    ...
    try:
        ...
        if ai.get("action") != "skip" and float(ai.get("confidence", 0.0) or 0.0) >= AUTO_SEND_THRESHOLD:
            if AUTO_SEND_HIGH_CONFIDENCE:
            ...
            elif AUTO_INVITE_HIGH_CONFIDENCE:
                inv = await invite_lead_to_group(client, lead["id"])
                await send_admin_notice(client, f"🤖 AUTO_INVITE {lead['id']}: {inv}")
